...
<xsl:stylesheet version="1.0" 
	...
	<xsl:variable name="versjon" select="'rekvisisjon1.5 - v3.1.2 '"/>
	...
	<xsl:template match="lso:ServReq">
		...
		<xsl:if test="lso:NoReport or lso:LevelOfDetail or lso:EscortRequired or lso:Permission">
			<tr>
				<xsl:if test="lso:NoReport">
					<th>Ønsker svar</th>
					<td width="{((($col)-2)*number(not(lso:LevelOfDetail | lso:EscortRequired | lso:Permission))+1)*$std-td}px" colspan="{(($col)-2)*number(not(lso:LevelOfDetail | lso:EscortRequired | lso:Permission))+1}">
						<xsl:choose>
				...
				<xsl:if test="lso:LevelOfDetail">
					...
					<td width="{((($col)-2-count(lso:NoReport)*2)*number(not(lso:EscortRequired | lso:Permission))+1)*$std-td}px" colspan="{(($col)-2-count(lso:NoReport)*2)*number(not(lso:EscortRequired | lso:Permission))+1}">
						<xsl:apply-templates select="lso:LevelOfDetail"/>
					</td>
				</xsl:if>
				<xsl:if test="lso:EscortRequired">
					<th>Ledsagelsesbehov</th>
					<td width="{((($col)-2-count(lso:NoReport | lso:EscortRequired)*2)*number(not(lso:Permission))+1)*$std-td}px" colspan="{(($col)-2-count(lso:NoReport | lso:EscortRequired)*2)*number(not(lso:Permission))+1}">
						<xsl:for-each select="lso:EscortRequired">
							<xsl:call-template name="k-8247"/>
						</xsl:for-each>
					</td>
				</xsl:if>
				<xsl:if test="lso:Permission">
					<th>Tillatelser</th>
					<td colspan="{($col)-1-count(lso:NoReport | lso:LevelOfDetail | lso:EscortRequired)*2}">
						<xsl:for-each select="lso:Permission">
